...
| PR #969, dashboard upload proxy, head `de745be3` | open, `develop` target; accessibility regression fixed on current head; required Checks and current review queued | Preserve one accessible upload action while keeping native file selection behavior | terminal browser/accessibility/security Checks, current review evidence, protected merge |
