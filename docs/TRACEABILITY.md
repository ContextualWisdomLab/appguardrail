...
The AppGuardrail obligation is the reusable causal pattern, not the workflow or issue title. `github-actions-transport-only-poll-bound` is deliberately bounded to conventional two-space GitHub Actions job syntax and literal block shell steps. The transport-failure budget, unbounded `while :` / `while true`, executable `gh api`, and direct `sleep` must occur in one `run: |` block, with the network poll and delay before that same loop's `done`. Evidence split across jobs, across run steps, or after the loop is negative; quoted `gh api`/`sleep` strings and shell comments are not executable evidence. The same polling job is negative when it has an explicit job timeout, or the loop contains a reviewed fail-closed total deadline/attempt guard that exits nonzero. Safety evidence is local: a timeout, deadline, or attempt counter in a sibling job cannot suppress a distinct vulnerable poll, and a comparison that only logs without terminating is not a safety guard.

Regression evidence lives in `tests/test_github_actions_poll_bounds.py` and the pinned answer-free source fixtures under `tests/fixtures/security_corpus/github_actions_transport_only_poll_{vulnerable,fixed}.yml`. Current false-negative boundaries include cross-file/composite-action polling, non-shell control flow, dynamically generated workflows, quoted/nonstandard job-key syntax, noncanonical YAML indentation, shell loops whose structure differs materially from the reviewed `while ... done` form, and unrelated retry frameworks. These remain explicit obligations rather than implied coverage. The detector remains active-PR evidence until its exact head passes required checks and ordinary protected integration; the already-protected `.github` repair is prevention/control-plane evidence only and does not itself satisfy AppGuardrail scanner coverage.
