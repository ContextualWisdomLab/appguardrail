...
| authenticated workflow-result detector evidence | `scripts/ci/collect_org_security_failures.py` + `appguardrail_core/source_evidence.py` + IssueOps/control-plane persistence | active-issue-938; exact protected-head promotion pending |
...
## Source-bound workflow traceability

The production chain is:

`GitHub Actions REST run/job` → `source_evidence.acquire_workflow_evidence` →
`build_source_bound_finding` → `IssueOps/control-plane normalized finding`.

The source-bound envelope records `probe_ref`, `acquirer_ref`, repository,
revision, generated artifact identity, source SHA-256, observation/acquisition
times, typed assessment, and evidence digest. The legacy collector envelope is
retained only for compatibility. A failed job is a control failure and cannot
