...
| automatic scanner detection of CSV formula leading-whitespace neutralization bypass | built-in `javascript-csv-formula-leading-whitespace-bypass` rule | active detector PR for the tested ScopeWeave PR #386 JavaScript/TypeScript audit-CSV source shape; promote only after protected-head merge evidence |
...
## CSV formula-neutralization traceability contract

For spreadsheet-facing CSV formula-prefix bypasses, trace separately:

1. the source repository, PR, vulnerable head/blob, and reviewed fixed head/blob;
2. the exact formula-prefix guard and the CSV output sink it is intended to protect;
3. the source-derived vulnerable replay and reviewed negative replay;
4. the packaged detector rule and bounded CSV-context prefilter;
5. explicit not-claimed cases such as exporters with no recognizable guard;
6. production `_scan_file` execution with CWE-1236 evidence;
7. exact-head security, review, and merge evidence.

The active ScopeWeave PR #386 CSV detector is deliberately limited to the observed leading-whitespace neutralization bypass. OWASP documents broader delimiter, quote, control-character, full-width and spreadsheet-specific behavior; those are separate validation or detector obligations and are not silently promoted into this regex's efficacy claim.

## Standards/research

Existing repository docs/doctoring/security evidence remain the bibliography/source-of-truth for standards such as SARIF, CycloneDX, GitHub security interfaces, and applicable OWASP/CWE classes. Material new detector classes should add authoritative standard/CWE/OWASP references and APA 7 citations in doctoring where research/standards materially drive implementation. The CSV formula detector's references and limitations are recorded in `docs/detectors/javascript-csv-formula-whitespace.md`.
