...
def _collect_files(base_path: Path):
    ...
    # PERFORMANCE: Use os.scandir() instead of os.walk() to avoid expensive stat() calls.
    # os.scandir caches directory and file attributes (is_dir, is_file),
    # which makes file traversal significantly faster.
        ...
        try:
            with os.scandir(current_dir) as entries:
                for entry in entries:
                    # SECURITY: Avoid Path Traversal by skipping symlinks
                    if entry.is_symlink():
                        continue
                    if entry.is_dir():
