...
def cmd_scan(args):
    ...
    # ⚡ Bolt: Hoist expensive pathlib checks outside the scan loop
    # Path.is_dir() and Path.resolve() trigger stat() calls which are slow in a tight loop.
    ...
    for file_path in files_to_scan:
        ...
        file_findings = _scan_file(
            file_path, scan_path, resolved_base_path=resolved_base_path
        )
...
def _path_matches_glob_cached(path: str, pattern: str) -> bool:
    path = path.replace("\\", "/")
...
def _path_matches_glob(path: str, pattern: str) -> bool:
    ...
    # ⚡ Bolt: Use cached fnmatch to prevent slow evaluation of heavy globs in tight loops
    return _path_matches_glob_cached(path, pattern)
...
def _scan_file(file_path: Path, base_path: Path, **kwargs):
    ...
    resolved_base_path = kwargs.get("resolved_base_path")
