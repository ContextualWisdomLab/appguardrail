...
def cmd_scan(args):
    ...
    # ⚡ Bolt: Cache scan_path properties before the loop to avoid repeating them
    scan_path_is_dir = scan_path.is_dir()
    ...
    for file_path in files_to_scan:
        ...
        file_findings = _scan_file(
            file_path,
            scan_path,
            base_path_is_dir=scan_path_is_dir,
...
def _scan_file(file_path: Path, base_path: Path, **kwargs):
    ...
    # ⚡ Bolt: Extract cached pathlib checks from kwargs if available
    base_path_is_dir = kwargs.get("base_path_is_dir")
    if base_path_is_dir is None:
        base_path_is_dir = base_path.is_dir()

    base_path_is_file = kwargs.get("base_path_is_file")
    ...
    resolved_base_path_str = None
    resolved_base_path_prefix = None
                ...
                if include_paths or exclude_paths:
                    if rel_path_for_filters is None:
                        if resolved_base_path is None:
                            resolved_base_path = (
                                base_path if base_path_is_dir else Path(".").resolve()
                            )
