...
def cmd_scan(args):
    ...
    # ⚡ Bolt: Pre-compute base path resolutions to avoid massive redundant pathlib I/O
    # overhead for every single file scanned in large repositories.
    resolved_scan_path = scan_path if not scan_path_is_file else Path(".").resolve()
    resolved_scan_path_str = str(resolved_scan_path)
    resolved_scan_path_prefix = (
        resolved_scan_path_str + os.sep
        if not resolved_scan_path_str.endswith(os.sep)
        else resolved_scan_path_str
    )
    ...
    for file_path in files_to_scan:
        ...
        file_findings = _scan_file(
            file_path,
            scan_path,
            resolved_base_path=resolved_scan_path,
            resolved_base_path_str=resolved_scan_path_str,
            resolved_base_path_prefix=resolved_scan_path_prefix,
...
def _scan_file(
    ...
    if resolved_base_path is None:
        ...
        resolved_base_path = base_path if not base_path_is_file else Path(".").resolve()
