...
def _load_packaged_regex_rules():
    ...
    loaded = []
    try:
        rule_files = sorted(importlib.resources.files("scanner.rules").iterdir())
...
def dashboard_index_path():
    ...
    importlib.resources.
    """
    return Path(str(importlib.resources.files("scanner").joinpath("dashboard", "index.html")))


def dashboard_tokens_path():
    """Locate the canonical design-token source shipped with the package."""
    return Path(str(importlib.resources.files("scanner").joinpath("dashboard", "tokens.json")))
