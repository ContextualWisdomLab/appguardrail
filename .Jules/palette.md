...
## 2024-07-28 - Dialog and Table Row Accessibility
**Learning:** For accessibility in native HTML tables, interactive table rows, and `<dialog>` elements, specific ARIA attributes are necessary for proper screen reader behavior.
**Action:** In `scanner/dashboard/index.html`, interactive table rows (`<tr>`) must use `aria-haspopup="dialog"`. Header cells (`<th>`) must use `scope="col"`. `<dialog>` elements must use `aria-labelledby` to link to a dynamically injected title ID (e.g., `dlg-title`).
