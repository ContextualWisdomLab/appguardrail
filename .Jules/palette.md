...
## 2026-07-22 - Native HTML/JS Dashboard Accessibility
**Learning:** When building single-file vanilla HTML/JS dashboards, dynamically generated interactive elements (like table rows and dialogs) often miss core accessibility attributes that screen readers rely on to interpret complex UI structures properly.
**Action:** Always ensure that native `<dialog>` elements have an `aria-labelledby` linking to their dynamically injected title ID, table headers (`<th>`) explicitly declare `scope="col"`, and interactive rows (`<tr>`) that act as buttons specify `aria-haspopup="dialog"` to properly signal their behavior to assistive technologies.
