...
## 2026-07-21 - Clickable table row accessibility via tabindex and keydown
**Learning:** Adding `onclick` to non-interactive elements like `<tr>` excludes keyboard users who cannot tab to or trigger the element.
**Action:** When making rows clickable, explicitly add `tabindex="0"`, a semantic `role="button"`, a descriptive `aria-label`, a focus-visible outline, and a `keydown` listener for `Enter` and `Space`.
