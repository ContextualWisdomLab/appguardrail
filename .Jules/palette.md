...
**Learning:** For screen readers, data tables lack clear associations without explicit row or header scope, and interactive elements that open modals need `aria-haspopup="dialog"`. Also, dynamically populated `<dialog>` elements must correctly link their programmatic titles via `aria-labelledby` referencing a dynamically injected ID (e.g. `dlg-title`).
**Action:** When creating or modifying data tables, ensure interactive table rows use `aria-haspopup="dialog"`, header cells use `scope="col"`, and `<dialog>` elements use `aria-labelledby` to link to a dynamically injected title ID.
