...
## 2026-07-16 - Enhance Table and Dialog Accessibility
**Learning:** When using native tables and dialogs, screen readers need explicit cues to associate data properly and announce interactive elements. Adding `scope="col"` to `<th>` tags explicitly connects columns with headers, and adding `aria-haspopup="dialog"` to clickable rows lets users know what action will occur. Also, native `<dialog>` elements need an accessible name via `aria-labelledby` linking to the dialog's title to be properly identified by screen readers.
**Action:** For all future data tables, ensure `scope="col"` or `scope="row"` is used in table headers. For custom interactive rows opening overlays, always include `aria-haspopup`. Give all `<dialog>` elements an accessible name using `aria-labelledby`.
