...
def test_perf():
    include = ("src/**/*.py", "lib/**/*.py", "test/**/*.py")
    exclude = ("tests/**/*.py", "vendor/**/*.py")

    start = time.time()
    for i in range(1000000):
        _path_allowed_by_rule_cached(f"src/file{i%2000}.py", include, exclude)
    print(f"Original: {time.time() - start:.4f}s")

    start = time.time()
    for i in range(1000000):
        fast_path_allowed_by_rule_cached(f"src/file{i%2000}.py", include, exclude)
