...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "A successful same-head manual workflow_dispatch Strix or OpenCode run may supersede a stale failed PR statusCheckRollup context only when failed-check evidence explicitly lists it under Superseded failed checks with the exact target URL" "opencode review prompt allows only explicit same-head manual evidence to supersede stale rollup failures"
	...
	assert_file_contains "$workflow_file" "manual workflow_dispatch Strix or OpenCode run may supersede a stale failed PR statusCheckRollup context" "opencode review prompt covers both Strix and OpenCode manual superseding evidence"
