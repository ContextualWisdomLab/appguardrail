...
run_pull_request_target_unchanged_line_finding_allows_baseline_case() {
	local report_line="$1"
...
printf '%s\n' "${STRIX_LLM:-}" >> "${FAKE_STRIX_CALL_LOG:?}"
mkdir -p "${STRIX_REPORTS_DIR:?}/line-level/vulnerabilities"
cat >"$STRIX_REPORTS_DIR/line-level/vulnerabilities/vuln-0001.md" <<EOS
Severity: CRITICAL
Location 1:
scanner/cli/vibesec.py:${FAKE_STRIX_REPORT_LINE:?}
EOS
echo "Penetration test failed: line-level changed-file finding"
exit 1
EOF
	chmod +x "$fake_strix"
...
EOS
	...
	(
		cd "$repo_root_dir"
		cat >>"$changed_file" <<'EOS'
NEW_RULE = {"id": "python-insecure-deserialization"}
EOS
		git add .
		git commit -qm 'head commit'
	)
	...
	(
		...
		env -u GITHUB_EVENT_PATH \
			...
			STRIX_TEST_CHANGED_FILES_OVERRIDE="$changed_file" \
			FAKE_STRIX_CALL_LOG="$call_log" \
			FAKE_STRIX_REPORT_LINE="$report_line" \
	...
	assert_equals "1" "$(wc -l <"$call_log" | tr -d ' ')" "case=$case_name strix call count"
...
run_pull_request_target_unchanged_line_finding_allows_baseline_case \
	...
	"pull-request-target-unchanged-line-finding-allows-baseline"
