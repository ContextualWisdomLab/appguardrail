...
The hourly workflow now invokes OpenCode directly, so issue labels are no longer
an agent handoff mechanism. This compatibility command remains read-only: it can
recover the same active issue identity for operators and tests without mutating
GitHub state or touching review-agent credentials.
"""
...
def reconcile_handoff(
    ...
    *,
    ...
    """Return the current PR-first or active-gap state without mutation."""
    del dry_run
