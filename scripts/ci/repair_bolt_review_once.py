...
""",
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        if has_sensitive_header and (
            original.scheme.lower() != "https" or target.scheme.lower() != "https"
        ):
            raise urllib.error.URLError("Authenticated redirects require HTTPS")

        redirected = super().redirect_request(req, fp, code, msg, headers, newurl)
        if redirected is None or not has_sensitive_header:
            return redirected
...
def test_authenticated_redirect_rejects_https_downgrade() -> None:
