...
def extract_public_references(message: str) -> tuple[str, ...]:
    ...
    seen = {}
    # ⚡ Bolt: Use an explicit loop instead of dict.fromkeys(generator) to avoid
    # significant generator overhead and frame allocation in hot paths.
...
def _merge_references(*groups: tuple[str, ...]) -> tuple[str, ...]:
    seen = {}
    # ⚡ Bolt: Use explicit loops instead of dict.fromkeys(generator) to avoid
    # significant generator overhead and frame allocation in hot paths.
    for group in groups:
