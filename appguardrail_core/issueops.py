...
def is_failure(conclusion: str | None) -> bool:
    """Return whether a GitHub conclusion represents a failing run."""
    return (conclusion or "").lower() in FAILURES


def is_security_name(*names: str | None) -> bool:
    """Return whether workflow or job names look security related."""
...
def parse_run_url(url: str) -> tuple[str, int]:
    """Extract repository full name and run id from a GitHub Actions URL."""
...
def sanitize_label_value(value: str) -> str:
    """Convert arbitrary repository text into a bounded GitHub label suffix."""
    value = re.sub(r"[^A-Za-z0-9._:-]+", "-", value.strip()).strip("-")
    return value[:45] or "unknown"


def redact(log: str) -> str:
    """Remove ANSI noise, timestamps, and likely secrets from job logs."""
...
def log_ranges(lines: list[str], patterns: list[re.Pattern[str]]) -> list[tuple[int, int]]:
    """Return contextual line windows around matching log lines."""
    return [
...
def compress_log(log: str, max_lines: int = DEFAULT_MAX_LOG_LINES, max_chars: int = DEFAULT_MAX_LOG_CHARS) -> str:
    """Return a bounded, redacted snippet that preserves failure reasons."""
...
def seen_key(finding: dict[str, Any]) -> str:
    """Build the de-duplication key for one workflow job finding."""
    return f"{finding['run_id']}:{finding['job_id']}"


def marker(repo: str, workflow: str, seen: set[str]) -> str:
    """Render the hidden issue marker that tracks already-published jobs."""
...
def parse_marker(body: str | None) -> dict[str, Any]:
    """Parse a hidden AppGuardrail marker from an issue body."""
    body = body or ""
...
def replace_marker(body: str | None, repo: str, workflow: str, seen: set[str]) -> str:
    """Insert or replace the hidden AppGuardrail marker in an issue body."""
    body = body or ""
...
def title(finding: dict[str, Any]) -> str:
    """Build the stable IssueOps title for a security workflow failure."""
    return f"[security-failure] {finding['repo']}: {finding['workflow']}"


def summary(finding: dict[str, Any]) -> str:
    """Render the metadata block shown in issues and follow-up comments."""
...
def issue_body(finding: dict[str, Any], seen: set[str]) -> str:
    """Render a new issue body with metadata and compressed job log evidence."""
...
def issue_comment(finding: dict[str, Any]) -> str:
    """Render a follow-up comment for a newly observed failing job."""
