...
- Detect direct or module-qualified `os.system(...)` calls and `subprocess`
  calls whose `shell` keyword is not statically false.
...
- Preserve the legacy regex only as a fail-closed fallback for syntax-invalid
  Python and retain exact source offsets across UTF-8 text, Python newline
  boundaries, form feeds, and Unicode line separators.
