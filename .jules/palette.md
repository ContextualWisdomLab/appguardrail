...
## 2024-07-22 - Remove aria-label from tr elements
**Learning:** Adding `aria-label` to `<tr>` elements (even with `role="button"`) overrides natural table cell reading and hides critical table data from screen reader users.
**Action:** Do not use `aria-label` on table rows; allow screen readers to read the table content naturally.
