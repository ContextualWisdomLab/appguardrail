...
## 2026-09-17 - Native Disabled State for Interactive Non-button Elements
**Learning:** Unlike the native `:disabled` attribute, `aria-busy="true"` only conveys 'work in progress' to screen readers and does not natively prevent interactions or convey an unavailable state. Relying solely on `pointer-events: none` is an accessibility anti-pattern because it fails to prevent keyboard interactions (Enter/Space) and completely swallows mouse events, preventing mobile screen readers from perceiving the element's focus state.
**Action:** When disabling non-native interactive elements (like `<tr role="button">`) during loading states, always apply `aria-disabled="true"` alongside visual CSS disabling (e.g., `pointer-events: none` and `opacity`) and explicit JavaScript event guards on click/keydown listeners.
