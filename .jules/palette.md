...
**Learning:** Screen reader users can become disoriented when clicking an external link opens a new tab (`target="_blank"`) without any prior warning or programmatic indication.
**Action:** When using `target="_blank"` for external links, always explicitly warn screen reader users of the context switch by appending text like `(opens in a new tab)` to the `aria-label` or using visually hidden text.
