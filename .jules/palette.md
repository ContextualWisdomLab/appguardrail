...
**Learning:** To comply with WCAG G201 for `target="_blank"` external links, explicitly provide **both** an assistive-technology warning (e.g., visually hidden text like `(opens in a new tab)` linked via `aria-describedby`) and a visible warning (e.g., an external link SVG icon). Hidden text alone is insufficient.
**Action:** Always add a visually hidden span with an id and use `aria-describedby` on the anchor, and append an external link icon inside the anchor for visual indication.
