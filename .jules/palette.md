...
## 2024-09-20 - Disable Interactive Elements properly during Loading
**Learning:** aria-busy="true" does not prevent interaction. To truly block clicks while maintaining accessibility state, apply aria-disabled="true", use CSS pointer-events:none, and add explicit javascript condition checks to onclick/keydown listeners.
**Action:** Always pair aria-disabled="true" with event guards on non-native interactive elements when they shouldn't be interacted with during loading.
