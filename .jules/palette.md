...
## 2024-05-23 - Accessibility of transient loading states
**Learning:** Relying solely on CSS `pointer-events: none` or just setting `aria-busy="true"` on non-native interactive elements (like `<tr role="button">`) is an accessibility anti-pattern. `aria-busy` only conveys "work in progress" but does not disable interactions, and `pointer-events: none` fails to prevent keyboard interactions.
**Action:** Always apply `aria-disabled="true"` alongside visual CSS disabling (`pointer-events: none`) and add explicit JavaScript event guards on click/keydown listeners to properly disable non-native interactive elements during transient loading states.
