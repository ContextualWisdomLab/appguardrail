...
## 2024-05-24 - Proxy Input Elements in Zero-Build Dashboards
**Learning:** Replacing native UI elements (like `<input type="file">`) with styled proxy elements (like `<button>`) must strictly adhere to the "label-in-name" principle. If the visible text on the proxy button fully describes the action, a redundant `aria-label` is unnecessary and should be removed from the proxy element to prevent duplicate accessible names. Additionally, replacing the input element structure means any UI contract tests that assert exact HTML structures (e.g., `aria-label="Upload findings file"`) will break and must be updated to match the new proxy button structure.
**Action:** Always verify UI contract tests via `grep` before replacing native elements with proxies. When implementing the proxy pattern, ensure the hidden native element and the visible proxy button do not share redundant accessible names, and update contract tests to assert against the proxy's visible text.
