...
## 2026-08-14 - External Links Accessibility
**Learning:** Using `target="_blank"` on links without warning screen reader users causes unexpected context switches that can be disorienting and break navigation flow.
**Action:** Always include an `aria-label` with text like "(opens in a new tab)" or visually hidden text when rendering external links to preserve a predictable screen reader experience.
