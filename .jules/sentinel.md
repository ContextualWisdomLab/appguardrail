...
## 2026-07-17 - Fix missing redaction tokens for slack webhook rules
**Vulnerability:** New secret rules like `hardcoded-slack-webhook-url` lacked matching tokens in `_SENSITIVE_RULE_TOKENS`, resulting in matched webhook strings being printed or outputted without redaction.
**Learning:** Even if a rule implies a secret context (like webhooks or tokens), unless a keyword explicitly exists in the `_SENSITIVE_RULE_TOKENS` tuple (such as "slack" or "webhook"), the CLI scanner won't suppress the output using `[REDACTED: sensitive match suppressed]`. This exposes sensitive configurations in plain text to CI logs or local developers' consoles.
**Prevention:** Adding "slack" and "webhook" to `_SENSITIVE_RULE_TOKENS`. Whenever we add custom or new secret detection rules to `secrets.yml`, we must write tests verifying that their unique ID strings trigger `_is_sensitive_rule()`.
