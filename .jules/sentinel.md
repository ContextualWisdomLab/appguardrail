...
## 2024-05-18 - Fix DOM XSS and Prototype Pollution in Console Dashboard
**Vulnerability:** Untrusted string properties from JSON payloads (e.g., `s.id`, `f.severity`, numeric stats) were injected into DOM via `innerHTML` and used as object keys for map lookups (`SEV[f.severity]`).
**Learning:** `innerHTML` sanitization is often incomplete when properties appear safe (like numeric totals or status strings), allowing payloads like `__proto__` to pollute mappings, or strings to bypass type assumptions.
**Prevention:** Always escape string payloads using `esc()`. Coerce expected numeric payloads strictly using `Number()` (which safely resolves to `NaN` for non-numeric strings) instead of escaping them. Normalize and coerce mapping keys (e.g., `String(val).toUpperCase()`) before object lookups to avoid prototype object access attacks.
