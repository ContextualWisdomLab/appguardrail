...
## 2026-09-16 - 빈 호스트명(Empty hostname)을 이용한 SSRF 우회 방지
**Vulnerability:** SSRF 필터인 `_is_safe_url`에서 빈 호스트명(`http://` 또는 `http://user@` 등)에 대한 검증이 누락되어, DNS 확인 단계를 우회하거나 예기치 않은 오류를 유발할 수 있습니다.
**Learning:** Python의 `urlparse`는 `http://user@`와 같은 URL을 분석할 때 스킴(scheme)과 사용자 정보(user info)는 인식하지만 호스트명은 빈 문자열로 처리합니다. 명시적으로 빈 호스트명을 거부하지 않으면 빈 문자열이 정상적으로 확인되거나 필터링되지 않아 기본 IP 차단 검증을 우회하게 됩니다.
**Prevention:** URL 파싱 후 DNS 확인이나 IP 검증 단계로 넘어가기 전, 항상 명시적으로 호스트명이 비어있는지(`if not host: return False`) 확인하고 거부해야 합니다.
