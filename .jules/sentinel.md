...
## 2026-09-19 - [Reject inherited severity-map properties]
**Vulnerability:** The console read an untrusted severity as a key from a normal JavaScript object. Keys such as `constructor` and `toString` could therefore resolve through `Object.prototype` and be interpolated as invalid CSS values. This was a prototype-chain lookup, not prototype mutation or pollution.
**Learning:** Case normalization changes spelling but does not express the security invariant. A lookup table is safe only when the requested key is an own property of that table.
**Prevention:** Normalize the severity once, require `Object.prototype.hasOwnProperty.call(SEV, severity)`, and otherwise use the fixed INFO color. Keep the visible severity value HTML-escaped.
