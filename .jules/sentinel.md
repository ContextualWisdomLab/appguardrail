...
## 2025-02-14 - XSS Escape Regex Backticks
**Vulnerability:** DOM XSS via backticks (`)
**Learning:** The custom HTML escape function in `console.html` missed escaping backticks (`` ` ``). While not commonly a direct attack vector in modern browsers outside of specific attributes, failing to escape them when constructing template literals or embedding in certain contexts can lead to XSS.
**Prevention:** Always include backticks when writing custom HTML escaping routines or prefer using built-in browser primitives (e.g. `textContent`).
