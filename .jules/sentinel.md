...
## 2024-05-23 - DOM XSS & Prototype Pollution in Dashboard
**Vulnerability:** DOM XSS via unescaped string properties (IDs) and lack of type coercion for numeric metrics, plus Prototype Pollution via untrusted strings in object mappings.
**Learning:** Template literals building HTML without framework protection are highly susceptible to XSS if inputs aren't strictly coerced (e.g. Number()) or escaped (e.g. esc()).
**Prevention:** Always escape string properties, coerce numeric properties with Number(), and normalize strings used as object keys.
