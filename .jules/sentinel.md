...
## 2026-07-29 - [SSRF via HTTP redirect bypass in urlopen]
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass due to `urllib.request.urlopen` automatically following HTTP redirects (e.g., 301, 302, 307, 308). Even if the initial URL is validated to not point to internal IP ranges, the remote server can respond with a redirect to an internal IP or local file (e.g. `file://`, `127.0.0.1`), which `urlopen` will follow natively, bypassing SSRF protections on the initial URL.
**Learning:** Initial URL validation is not enough if the HTTP client automatically follows redirects to new, unvalidated URLs. Python's default `urllib.request.urlopen` behaves exactly like this.
**Prevention:** To prevent SSRF bypasses via redirects, construct a custom opener using `urllib.request.build_opener()` and pass in a `urllib.request.HTTPRedirectHandler` that explicitly overrides `redirect_request` to return `None`, effectively blocking redirects. Then, use `opener.open()` instead of `urlopen()`.
