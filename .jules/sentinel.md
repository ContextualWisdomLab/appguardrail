...
## 2026-07-29 - [Stored SSRF vulnerability via empty hostnames]
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass due to `urlsplit` properly parsing URLs with empty hostnames (e.g., `http://`, `http://user@`) and resolving as safe when passed through without checking hostname length, since local DNS resolution failures (`socket.gaierror`) were ignored in `_is_safe_url`.
**Learning:** `socket.getaddrinfo` throws `gaierror` when given an empty hostname instead of explicitly stating it's internal. If `gaierror` is ignored or handled silently (to allow unresolvable, external dummy domains for testing), an empty hostname effectively bypasses the validation and will cause downstream `urlopen` to raise an `AttributeError` on storage, or hit unexpected loopbacks.
**Prevention:** When validating URLs to prevent SSRF in Python, explicitly verify the length and truthiness of the extracted hostname string (`if not host: return False`) prior to attempting DNS resolution and filtering IPs.
