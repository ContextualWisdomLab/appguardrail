...
## 2026-07-25 - SSRF via HTTP Redirects
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass due to incomplete network protections against HTTP redirects when using `urllib.request.urlopen`.
**Learning:** Checking the initial URL provided by the user with `_is_safe_url` prevents direct SSRF, but `urlopen` follows HTTP redirects automatically without validating the target URL of the redirect. This allows an attacker to bypass the protection by providing a safe initial URL that redirects the HTTP client to an unsafe internal IP or metadata endpoint.
**Prevention:** Rather than using `urlopen`, build a custom opener with `urllib.request.build_opener` that uses a custom `HTTPRedirectHandler`. This custom handler should override `redirect_request` and validate the new target URL against `_is_safe_url` before allowing the redirect to proceed.
