...
## 2024-05-18 - [Fix XSS via untrusted JSON payload in UI]
**Vulnerability:** Untrusted JSON string properties and numeric properties were dynamically interpolated into HTML without escaping or type coercion.
**Learning:** Even internal properties (like IDs, counts) can carry XSS payloads if ingested from unvalidated external JSON. String sanitizers are needed for strings, but numeric fields should use strict type coercion (`Number()`).
**Prevention:** Always escape string-based properties (like IDs) using `esc()` and enforce numeric properties using `Number()` when interpolating into `innerHTML`.
