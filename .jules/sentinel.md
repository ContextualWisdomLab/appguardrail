...
## 2024-11-20 - 카테고리 스푸핑 관련 테스트 수정
**Learning:** `category_for_references` 오버라이드 로직을 제거할 때, 기존 휴리스틱 함수(`_finding_category`)가 룰 ID를 올바른 카테고리로 분류하지 못하고 있던 문제를 파악해야 합니다. 특정 룰 ID(예: `python-stored-ssrf-webhook-url`)가 부적절하게 `payment`로 분류될 수 있습니다.
**Action:** 보안 분류 로직(`_finding_category`)에서 `"ssrf"` 패턴을 올바르게 감지하도록 수정하여 기존에 오버라이드로 덮어씌워지던 문제를 근본적으로 해결하십시오.
