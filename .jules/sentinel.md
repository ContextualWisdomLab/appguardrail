...
## 2026-08-01 - Fix SSRF bypass via empty hostname
**Vulnerability:** URLs with empty hostnames (e.g., `http://` or `http://user@`) bypassed the `_is_safe_url` checks because `socket.getaddrinfo("")` fails with a `socket.gaierror`, which was caught and ignored by the `try...except` block, leading the function to incorrectly return `True`. This could allow attackers to bypass SSRF protections, as `urllib.request.urlopen` (or the underlying system resolution) might resolve empty strings to local addresses like `127.0.0.1` or `0.0.0.0`.
**Learning:** DNS resolution functions and URL parsers can have inconsistent behavior with edge-case inputs like empty strings. Catching and silently passing on `gaierror` without explicit validation of the input string leaves a blind spot for these malformed payloads.
**Prevention:** Always explicitly check for and reject empty hostnames (`if not raw: return False`) immediately after parsing the URL and before attempting any DNS resolution or IP validation.
