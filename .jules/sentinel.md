...
## 2026-07-30 - DOM XSS via unescaped string interpolation in console.html
**Vulnerability:** DOM XSS via unescaped numeric/ID properties interpolated into `innerHTML` in `scanner/dashboard/console.html`.
**Learning:** Properties expected to be numbers or identifiers from an API response (like `total`, `deploy_blocking`, `id`) were directly interpolated into HTML without escaping or coercion. An attacker manipulating the API response could inject malicious scripts.
**Prevention:** Always explicitly use the `esc()` sanitizer for string-based properties (like `id`) or use strict `Number()` coercion for numeric fields (like `total`) when dynamically rendering HTML from untrusted JSON payloads to neutralize XSS vectors.
