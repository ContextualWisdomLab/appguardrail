...
## 2026-07-21 - Add missing tokens to _SENSITIVE_RULE_TOKENS
**Vulnerability:** Several sensitive secrets (Slack webhooks, Twilio credentials, npm tokens, PyPI tokens, SendGrid API keys) could be exposed in terminal output or CI logs because their matching rule tokens were missing from `_SENSITIVE_RULE_TOKENS`.
**Learning:** `_SENSITIVE_RULE_TOKENS` must stay precisely synchronized with all rule IDs defined in `scanner/rules/secrets.yml`. Without their exact keyword substring (like `slack`, `twilio`, or `npm`), the redactor `_safe_snippet` will fail to mask the discovered strings.
**Prevention:** Whenever new integrations or secret tokens are added to `scanner/rules/secrets.yml`, strictly update `_SENSITIVE_RULE_TOKENS` in `scanner/cli/appguardrail.py`. Write a test that verifies `_is_sensitive_rule` evaluates to True for every new rule ID containing a secret.
