...
**Vulnerability:** A Server-Side Request Forgery (SSRF) bypass due to Python's `urllib.request.urlopen` implicitly following HTTP redirects.
**Learning:** Checking the initial URL via `_is_safe_url` is insufficient if the external server redirects the request to an internal, link-local, or loopback IP address, which bypasses the initial URL validation.
**Prevention:** Always use a custom HTTP redirect handler using `urllib.request.build_opener(urllib.request.HTTPRedirectHandler)` that validates `newurl` prior to allowing the redirect.
