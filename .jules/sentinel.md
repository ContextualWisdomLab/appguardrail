...
## 2026-08-20 - Stored XSS via unescaped scan ID in data attributes
**Vulnerability:** A stored Cross-Site Scripting (XSS) vulnerability was found in `scanner/dashboard/console.html` where a scan ID from the API (`s.id`) was directly interpolated into the `data-id` HTML attribute without escaping.
**Learning:** Even internal API IDs must be escaped before interpolation in HTML contexts. An attacker could register a malicious scan ID like `" onmouseover="alert('XSS')` to break out of the `data-id` attribute and execute arbitrary JavaScript when the user interacts with the element.
**Prevention:** Always use the `esc()` function when interpolating variables from JSON payloads into HTML strings, including non-visible attributes like `data-*` fields, to ensure special characters like double quotes are properly encoded.
