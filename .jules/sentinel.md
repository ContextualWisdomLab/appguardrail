...
## 2024-05-01 - [CRITICAL] 파이썬 커맨드 인젝션 취약점 점검 강화
**Vulnerability:** 기존 `python-command-injection` 정규식은 `os.system` 함수가 `shell=True` 인자와 함께 호출될 때만 인젝션 취약점으로 식별하고, `shell=True` 파라미터가 없거나 `shell=1`을 사용할 때는 점검을 우회하는 문제가 있었습니다.
**Learning:** `os.system`은 본질적으로 시스템 쉘을 호출하기 때문에 `shell=True` 여부와 관계없이 입력값 검증 없이 사용할 경우 인젝션 위험이 존재합니다. 또한 `subprocess` 내 `shell=1` 옵션 또한 정수 1이 True로 평가되므로 동일한 보안 위험을 가집니다.
**Prevention:** 정적 분석 도구 규칙을 설계할 때는 `os.system`과 같이 기본적으로 쉘을 사용하는 함수와 `shell=True`, `shell=1` 같이 암시적으로 참으로 평가되는 인자들을 모두 포함하도록 정규식을 작성해야 합니다.
