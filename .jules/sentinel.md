...
## 2026-08-01 - [빈 호스트네임을 통한 SSRF 우회 취약점]
**Vulnerability:** URL에 빈 호스트네임(`http://` 또는 `http://user@`)이 포함되어 있을 때 `_is_safe_url` 함수가 이를 안전한 URL로 잘못 인식하여 SSRF(Server-Side Request Forgery) 검증을 우회할 수 있는 취약점이 존재했습니다.
**Learning:** `urllib.parse.urlparse`를 사용할 때 호스트네임이 없으면 `parsed.hostname`은 `None`을 반환합니다. 이를 빈 문자열 `""`로 처리하면 `ipaddress`나 `socket.getaddrinfo`에서 예외가 발생하는데, 예외 처리 로직에 의해 조기 반환(`return False`)이 되지 않고 함수 끝에 도달하여 검증이 통과(`return True`)되는 논리적 결함이 발생할 수 있습니다.
**Prevention:** 네트워크 요청을 위한 URL을 검증할 때, 파싱된 호스트네임이 실제로 존재하는지(예: `if not host: return False`) 반드시 직접적으로 확인해야 하며, 예외 처리가 의도치 않은 검증 통과(Fail-open)를 유발하지 않도록 철저히 검증해야 합니다.
