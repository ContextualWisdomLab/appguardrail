...
## 2024-10-27 - [HIGH] Fix Stored XSS via unescaped JSON properties in HTML
**Vulnerability:** DOM Cross-Site Scripting (XSS) in `console.html` and `index.html` where numeric summaries (like counts, IDs, Array lengths) and template IDs were directly interpolated into HTML strings via `.innerHTML` without sanitization. Even numeric inputs and IDs could theoretically be manipulated depending on upstream data structures, leading to Stored XSS.
**Learning:** Never trust dynamic properties derived from external or stored JSON responses—even those implicitly expected to be integers or identifiers. `innerHTML` blindly parses these properties.
**Prevention:** Always use the `.esc()` HTML escaping utility for every dynamic property derived from JSON—including counts, loop indices, table IDs, and lengths—when manually constructing HTML strings for `innerHTML`.
