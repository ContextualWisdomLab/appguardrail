...
## 2026-07-29 - [빈 호스트 이름을 통한 Stored SSRF 취약점]
**Vulnerability:** `urlsplit`이 빈 호스트 이름(예: `http://`, `http://user@`)이 포함된 URL을 올바르게 파싱하지만, 추출된 호스트 이름의 길이를 확인하지 않고 통과시키는 문제로 인해 발생하는 SSRF 우회 취약점입니다. 테스트 도메인을 허용하기 위해 로컬 DNS 분석 실패(`socket.gaierror`)를 무시하는 로직(`_is_safe_url`)에서 빈 호스트 이름이 안전한 외부 도메인으로 잘못 평가되었습니다.
**Learning:** `socket.getaddrinfo`에 빈 호스트 이름을 전달하면 로컬(internal)임을 나타내지 않고 `gaierror`를 반환합니다. 만약 `gaierror`를 무시하거나 조용히 처리할 경우, 빈 호스트 이름은 검증을 우회하게 되어 다운스트림의 `urlopen`이 데이터를 저장할 때 `AttributeError`를 발생시키거나 예상치 못한 루프백을 호출할 수 있습니다.
**Prevention:** Python에서 SSRF를 방지하기 위해 URL을 검증할 때는, DNS 분석 및 IP 필터링을 시도하기 전에 추출된 호스트 이름 문자열의 길이와 진릿값을 명시적으로 확인(`if not host: return False`)해야 합니다.
