...
## 2026-07-29 - [False Positive: DOM XSS via unescaped stats metrics in console dashboard]
**Vulnerability:** I initially flagged a potential DOM XSS vulnerability in `scanner/dashboard/console.html` due to unescaped JSON properties (`s.total`, `#stats` metrics, `data-id`) dynamically injected into the DOM via `innerHTML`.
**Learning:** This was a false positive. While `innerHTML` combined with an unescaped expression is a common XSS pattern, it is not sufficient on its own. The vulnerability requires the payload to be attacker-controllable text reaching a parser-sensitive sink. In this specific case, the `id` is a SQLite row identity, and the metrics (`total`, `deploy_blocking`, `new_blocking`) are server-computed integer counts created during normalized ingest by a trusted backend projection (`list_scans`). Because these are strictly typed and provenanced as integers at the server level, an untrusted findings payload cannot coerce them into HTML syntax.
**Prevention:** The reusable false-positive boundary is provenance-based: always trace the data flow back to the source to confirm if an attacker can actually supply arbitrary string values to the cited metrics before declaring a DOM XSS vulnerability.
