...
## 2024-08-25 - Extracted Strix failure log patterns not captured by CI reporting scripts
**Vulnerability:** Job logs generated by Strix runs when a provider infrastructure issue is encountered (e.g. `Strix could not complete authoritative vulnerability analysis...`) weren't captured properly and parsed correctly into issues by `scripts/ci/collect_org_security_failures.py` because `PRIMARY_LOG_RE` pattern lists in `appguardrail_core/issueops.py` didn't consider these errors.
**Learning:** Hardcoded regular expressions mapping specific failure keywords or phrases in a CI script log parsing module miss unexpected errors and edge-cases that are not considered initially.
**Prevention:** Consider updating log parsing logic to map a larger amount of failure log contents from the job execution payload without relying on strict pattern matching to preserve robustness across future error log updates, or simply add new known patterns as encountered.
