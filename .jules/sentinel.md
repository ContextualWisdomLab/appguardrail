...
## 2026-09-21 - Prototype Pollution in Dashboard via Object Maps

**Vulnerability:** XSS/DOM Injection via prototype pollution in JavaScript dictionary lookups. In `scanner/dashboard/console.html`, properties like `f.severity` are read directly from untrusted JSON into an object dictionary via `SEV[f.severity]`, and then the returned value (or `color` property) is directly interpolated into a `style` attribute via `innerHTML`. Since `f.severity` comes from user data (a finding), passing `__proto__` causes `SEV['__proto__']` to return the Object prototype. If an attacker controls the schema, they could use this to inject a string from `Object.prototype.toString` into the style attribute, which might be exploitable or cause errors.

**Learning:** When using object mappings `const map = {A: 1, B: 2}` to resolve classes, styles, or labels dynamically from untrusted strings via `map[untrusted]`, the `untrusted` string can be `__proto__`, which resolves to `Object.prototype` (and evaluates truthy). When injected into `innerHTML` via template strings, JS will implicitly call `toString()`, injecting `[object Object]` into the DOM. If `toString` is polluted or other properties exist, this leads to DOM injection/XSS.

**Prevention:** Sanitize the key before lookup. Coerce the variable to a string and transform it (e.g., `.toUpperCase()`) *before* looking it up in the dictionary, as `String("__proto__").toUpperCase()` results in `"__PROTO__"`, which does not hit the `__proto__` prototype lookup.
