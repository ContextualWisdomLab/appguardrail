...
## 2026-07-21 - SSRF bypass via HTTP Redirects in urllib.request.urlopen
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass through HTTP redirects. Even when user-provided URLs were strictly validated for their initial scheme and IP, `urllib.request.urlopen` automatically follows HTTP redirects (e.g., 301, 302, 307, 308). An attacker could host a server that redirects a validated external request to internal infrastructure or loopback addresses, completely bypassing the initial validation.
**Learning:** Python's default `urllib.request.urlopen` blocks native file redirects but transparently follows HTTP redirects, breaking the boundary of URL validation check logic if the target is dynamically constructed or relies on user input. Runtime SSRF validation cannot assume the final destination is the initial URL.
**Prevention:** Explicitly block HTTP redirects when constructing network requests by passing a custom `urllib.request.HTTPRedirectHandler` (returning `None` from `redirect_request`) into `urllib.request.build_opener()` instead of using the default `urlopen`.
