...
## 2026-08-01 - Centralize webhook SSRF validation at storage boundary
**Vulnerability:** The HTTP webhook handler performed SSRF validation `_is_safe_url` but the actual database storage function `set_webhook` blindly accepted any string. Other internal calls could bypass the HTTP route and persist dangerous URLs, leading to stored SSRF.
**Learning:** Security validations (like SSRF and URL schema checks) should be enforced at the storage boundary (e.g. within `set_webhook`), not just at the HTTP boundary, to prevent internal logic from inadvertently persisting malicious state.
**Prevention:** Moved `_is_safe_url` validation inside `set_webhook` and normalized empty strings to `None`. The HTTP handler now simply catches `ValueError` thrown by `set_webhook` instead of performing the validation itself.
