...
## 2026-08-01 - [SSRF Bypass via Empty Hostname in URL Parser]
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass due to `urllib.parse.urlparse` generating an empty hostname for URLs like `http://` or `http://user@`. The `_is_safe_url` function failed to explicitly reject empty hostnames, and the subsequent `socket.getaddrinfo` call resulted in a `gaierror` which was implicitly caught and ignored, resulting in the function returning `True` (safe).
**Learning:** Network endpoint guardrails must not rely exclusively on exceptions from DNS resolution logic to block invalid formats. Attackers can craft URLs that do not trigger DNS resolution or fail DNS resolution in a way that bypasses fail-closed logic if those specific exceptions are swallowed (e.g. `gaierror`). Empty hostnames must be explicitly checked.
**Prevention:** Always validate that the parsed URL hostname is not empty (`if not host: return False`) before performing any DNS resolution or IP validation.
