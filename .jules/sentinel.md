...
## 2026-07-25 - Strict Redirect Blocking in HTTP requests for SSRF protection
**Vulnerability:** Even if the initial URL is thoroughly checked against private IP ranges and dangerous schemes to prevent SSRF and LFI, `urllib.request.urlopen` will automatically follow HTTP redirects (like 301, 302). An attacker can provide a URL they control which passes initial validation, and then have their server redirect the request to an internal network target (e.g. `http://169.254.169.254/`) or a dangerous scheme, completely bypassing the initial SSRF check.
**Learning:** Default HTTP client implementations in Python (`urlopen`, `requests`) often transparently follow redirects. Validating only the initial URL string is insufficient if the client might fetch a different URL on its own.
**Prevention:** To prevent SSRF via redirects in `urllib.request`, explicitly disable redirect following by using a custom `urllib.request.HTTPRedirectHandler` that overrides `redirect_request` to return `None`, and use `build_opener().open()` instead of the default `urlopen()`.
