...
## 2026-07-20 - SSRF via HTTP Redirects in urlopen
**Vulnerability:** Even if the initial URL is validated (e.g., using `_is_safe_url`) to prevent Server-Side Request Forgery (SSRF), `urllib.request.urlopen` will automatically follow HTTP redirects. This allows an attacker to provide a safe-looking external URL that simply redirects the application to an internal, malicious, or restricted target (e.g., `http://169.254.169.254` or `file:///etc/passwd`).
**Learning:** `urlopen`'s default behavior blindly follows redirects without re-evaluating the new target URL against security policies. Relying solely on pre-flight validation of the initial URL leaves the application vulnerable to redirect-based SSRF.
**Prevention:** Avoid using `urllib.request.urlopen` directly for fetching user-provided or external URLs. Instead, construct a custom opener using `urllib.request.build_opener` alongside a custom subclass of `urllib.request.HTTPRedirectHandler`. Override the `redirect_request` method to enforce the exact same validation (e.g., `_is_safe_url(newurl)`) on every redirect hop before it is followed.
