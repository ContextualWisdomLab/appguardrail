...
## 2025-03-01 - Proper Input Handling for DOM XSS and Prototype Pollution Prevention
**Vulnerability:** Untrusted external JSON attributes injected into stringly-typed JavaScript functions (innerHTML and object property lookups) could carry payload vectors if not properly coerced into correct types.
**Learning:** Security theater from incorrect sanitization. `esc()` provides XSS safety for generic strings, but strict `Number()` coercing safely strips HTML injection on numerical components whilst retaining its native property state. Also, implicitly converting variables used for Dictionary Mapping could resolve string manipulation injection on prototype mapping rendering `[object Object]` or potential downstream execution.
**Prevention:** Always use explicit type coercion (e.g. `Number()`, `String().toUpperCase()`) on untrusted external variables intended for primitive types before they are consumed by operations like `innerHTML` or Object key lookup arrays in buildless JS.
