...
## 2026-08-05 - [SSRF Bypass via Empty Hostname in URL Validation]
**Vulnerability:** The SSRF protection `_is_safe_url` incorrectly identified URLs with empty hostnames (e.g., `http://`, `http://user@`) as safe because `socket.getaddrinfo` on an empty string throws an exception which was caught and ignored.
**Learning:** Empty hostnames bypass DNS resolution and string processing logic if not explicitly checked. `urllib.parse.urlparse` sets `hostname` to `None` for such URLs, and passing an empty string or `None` to DNS functions can result in exceptions that mask the vulnerability.
**Prevention:** Always explicitly verify that `parsed.hostname` is not empty or `None` before proceeding with DNS resolution and IP validation logic in URL safety checks.
