...
## 2026-08-20 - DOM XSS via Unescaped Numeric and ID Properties
**Vulnerability:** DOM XSS was possible via seemingly safe numeric/ID properties (like `findingsText`, `blocking`, `n`, `s.id`, `s.total`) sourced from user input (findings file) and directly injected into `innerHTML` within `scanner/dashboard/console.html` and `scanner/dashboard/index.html`.
**Learning:** Even though properties like `total` or `id` are expected to be numbers or safe database identifiers, they must still be escaped. An attacker manipulating the JSON payload could inject malicious HTML structures or scripts instead. All dynamic data rendered into the DOM must be sanitized.
**Prevention:** Always use the `esc()` sanitizer for any dynamically rendered property from external payloads (such as JSON API responses or findings files), regardless of the expected schema type (e.g. `esc(blocking)` rather than `${blocking}`), to prevent DOM XSS vulnerabilities.
