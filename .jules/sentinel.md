...
## 2025-02-28 - [MEDIUM] URL 검증 시 빈 호스트명으로 인한 SSRF 취약점 수정
**Vulnerability:** URL 유효성 검사 로직(`_is_safe_url`)이 빈 호스트명(`http://` 또는 `http://user@` 등)을 올바르게 거부하지 못하는 SSRF 취약점이 있었습니다. DNS 확인(resolution) 과정에서 발생하는 예외가 무시되어 이러한 URL이 안전한 것으로 잘못 간주되었습니다.
**Learning:** `socket.getaddrinfo`와 같은 DNS 확인 함수는 빈 호스트명에 대해 무조건 실패하며(예: `socket.gaierror`), `_is_safe_url` 내의 `except socket.gaierror:` 절에 의해 이 오류가 의도치 않게 통과 처리되면서 취약점이 발생했습니다.
**Prevention:** 호스트명을 추출한 직후 DNS 확인 함수에 도달하기 전에 명시적으로 빈 문자열 여부를 검사하고 즉시 `False`를 반환하도록 안전하게 거부(fail-closed)해야 합니다.
