...
## 2023-10-10 - [Fix prototype pollution in object mapping]
**Vulnerability:** Untrusted string used directly as object key for mapping severity levels.
**Learning:** This allows prototype pollution payloads (`__proto__`) to bypass expected mappings or manipulate prototype chains when rendering DOM elements.
**Prevention:** Sanitize the input by coercing and normalizing it (e.g., `String(val).toUpperCase()`) before mapping untrusted string values to object keys.
