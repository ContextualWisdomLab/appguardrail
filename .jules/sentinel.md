...
## 2024-09-26 - [Defense-in-depth SSRF protection for Webhooks]
**Vulnerability:** The internal `set_webhook` database function did not validate SSRF on the incoming `webhook_url`. It relied solely on the HTTP routing layer to validate the URL using `_is_safe_url`.
**Learning:** If internal modules directly call `set_webhook` bypassing the HTTP routing, they could introduce an SSRF vulnerability by saving an unsafe URL directly to the database. Validations should be strictly enforced at the database boundary to provide defense-in-depth security. Furthermore, handling explicit clear requests where URL could be empty string `""` before validating it ensures the logic correctly differentiates between clearing and malicious input.
**Prevention:** Always validate potentially unsafe parameters directly at the storage boundary (e.g., inside the DB functions) before storing them, and catch `ValueError` in the presentation layer. Ensure empty strings used for explicit clears are normalized to `None` prior to validations.
