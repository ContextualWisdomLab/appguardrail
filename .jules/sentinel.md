...
## 2026-07-20 - SSRF bypass via urllib.request HTTP redirect following
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass due to `urllib.request.urlopen` automatically following HTTP redirects (e.g., 301, 302). This allowed an attacker to provide a safe URL that passes initial SSRF checks but redirects to an internal network address (e.g., `http://169.254.169.254/latest/meta-data`).
**Learning:** Even if an initial URL passes safety validation checks (like checking scheme and IP against private/loopback blocks), `urllib.request.urlopen` will silently follow redirects. The validation is not re-applied to the redirect target, bypassing the security guardrail.
**Prevention:** Explicitly block all redirects by passing a custom `urllib.request.HTTPRedirectHandler` (which returns `None` from `redirect_request`) into `urllib.request.build_opener()` instead of using the default `urlopen`. This stops the redirect chain before it can access internal infrastructure.
