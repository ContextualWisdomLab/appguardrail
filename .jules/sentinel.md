...
## 2026-07-29 - [DOM XSS via unescaped stats metrics in console dashboard]
**Vulnerability:** Several JSON response fields such as `deploy_blocking`, `new_blocking`, `total`, and specific properties mapped from API responses in `$("#stats").innerHTML` and the `pill()` component in `scanner/dashboard/console.html` were unescaped, allowing for potential DOM XSS.
**Learning:** Even fields that are generally numeric (e.g. `s.total`, `n` inside `pill()`) can be hijacked through an API response allowing attackers to break out of HTML contexts and inject DOM XSS payloads in dynamically rendered templates via `innerHTML`.
**Prevention:** Use standard sanitation/escaping helper functions (e.g., `esc()`) on *every* dynamically injected string (even those expected to be numeric) before passing them to UI rendering engines.
