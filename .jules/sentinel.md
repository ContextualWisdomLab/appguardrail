...
## 2026-08-22 - PATH Environment Variable Hijacking
**Vulnerability:** External tools (Trivy, Bandit, Ruff, Semgrep, ZAP, CodeGraph) were executed by resolving their paths using `shutil.which` without validating if the resolved path was in a trusted system directory.
**Learning:** `shutil.which` relies on the `PATH` environment variable. If an attacker can control or modify the `PATH` variable, they can place a malicious executable with the same name as a trusted tool earlier in the `PATH`, leading to arbitrary code execution when the application attempts to run the tool.
**Prevention:** Implement a `_secure_which` function that wraps `shutil.which` and explicitly validates that the resolved absolute path falls within a hardcoded whitelist of trusted system directories (e.g., `/usr/bin`, `/bin`, `/usr/local/bin`) before returning it.

## 2026-08-22 - PATH Environment Variable Hijacking via shutil.which
**Vulnerability:** External tools (Trivy, Bandit, Ruff, Semgrep, ZAP, CodeGraph) were executed by resolving their paths using `shutil.which` without validating if the resolved path was in a trusted system directory.
**Learning:** `shutil.which` relies on the `PATH` environment variable. If an attacker can control or modify the `PATH` variable, they can place a malicious executable with the same name as a trusted tool earlier in the `PATH`, leading to arbitrary code execution when the application attempts to run the tool.
**Prevention:** Implement a `_secure_which` function that wraps `shutil.which` and explicitly validates that the resolved absolute path falls within a whitelist of trusted system directories (e.g., `/usr/bin`, `/bin`, `/usr/local/bin`, and optionally `RUNNER_TEMP` for CI) before returning it.

## 2026-08-22 - PATH Environment Variable Hijacking via shutil.which
**Vulnerability:** External tools (Trivy, Bandit, Ruff, Semgrep, ZAP, CodeGraph) were executed by resolving their paths using `shutil.which` without validating if the resolved path was in a trusted system directory.
**Learning:** `shutil.which` relies on the `PATH` environment variable. If an attacker can control or modify the `PATH` variable, they can place a malicious executable with the same name as a trusted tool earlier in the `PATH`, leading to arbitrary code execution when the application attempts to run the tool. `RUNNER_TEMP` shouldn't be trusted in normal operation unless specifically scoped for CI shim execution logic in environments like GitHub Actions where tools modify environments (like setting up paths dynamically).
**Prevention:** Implement a `_secure_which` function that wraps `shutil.which` and explicitly validates that the resolved absolute path falls within a whitelist of trusted system directories (e.g., `/usr/bin`, `/bin`, `/usr/local/bin`). For CI tools that might temporarily mock or shadow binaries using `RUNNER_TEMP` and `RUNNER_TOOL_CACHE`, ensure those are conditionally trusted based on `GITHUB_ACTIONS` environment presence to maintain testability without compromising general execution security.
