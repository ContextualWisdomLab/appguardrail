...
## 2026-07-29 - Strict SSRF validation for empty hostnames
**Vulnerability:** Empty hostnames such as `http://` or `http://user@` were evaluated as True by `_is_safe_url` before DNS resolution, bypassing SSRF protections.
**Learning:** Checking for DNS resolution errors and relying on exceptions when resolving empty strings leads to security blind spots when those exceptions are explicitly caught and suppressed as DNS resolution failures. Unresolvable addresses should be allowed for tests, but an empty hostname is inherently unsafe and must be explicitly rejected before resolution.
**Prevention:** Always validate that the extracted host string is not empty before returning True or attempting DNS resolution to avoid bypasses via improperly structured URLs.
