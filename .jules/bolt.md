...
## 2024-07-25 - Pathlib relative_to Overhead in Inner Loops
**Learning:** `pathlib.Path.relative_to` can be extremely slow inside tight loops (like iterating over matched vulnerability findings or regex scans). It invokes multiple function calls and overheads compared to simple string operations. Profiling showed that it could take ~6.5 seconds for 100k iterations, whereas simple string slicing and prefix checking takes ~0.08 seconds (nearly a 100x speedup). The memory note confirming this edge case exactly matching "." instead of "" was critical to preserve correctness.
**Action:** When working on performance-critical sections with loops in Python (like file tree traversal or content finding), hoist string representations out of loops and replace pathlib operations with optimized standard string manipulations (`startswith`, `len()`, slicing, `os.sep`). Always implement correct string fallbacks for edge cases (e.g. exactly matching base path `"."`).
