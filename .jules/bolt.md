...
**Learning:** Chaining multiple array methods (`.map().filter().sort()`) on large datasets triggered by high-frequency events causes redundant memory allocations and iteration overhead. When refactoring `indexOf` lookups to O(1) object maps, strictly preserve original sorting behavior (e.g., unknown elements sorting to `-1`) to prevent regressions in dependent consumers. Avoid using ES2019+ features like `Object.fromEntries` or `??` in vanilla JS files that might be parsed by older engines or static analyzers.
**Action:** Use a single `for` loop with early returns and O(1) object lookups for sorting priorities to minimize execution passes, ensuring exact behavioral parity for missing keys using ES5-compatible syntax (`!== undefined`).
