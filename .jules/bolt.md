...
## 2024-12-05 - Generator expression overhead in hot loops
**Learning:** Passing a generator expression to a dictionary constructor (e.g., `dict.fromkeys(x for x in y)`) or using them in `any()` or `all()` creates generator frames that incur measurable allocation overhead in hot paths.
**Action:** Replace generator expressions with explicit nested `for` loops that manually populate dictionaries or use `break` statements. This bypasses the overhead of creating and evaluating generator objects, yielding a measurable constant-factor speedup.
