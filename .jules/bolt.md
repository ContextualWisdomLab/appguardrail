...
## 2026-07-03 - Optimize tech stack detection file scanning
**Learning:** Generating thousands of `pathlib.Path` objects inside tight loops for file scanning causes severe performance degradation due to object creation overhead.
**Action:** Process paths as raw strings and use fast C-level string methods (`s.rfind()`, `s.split()`, `s.lower()`) for parsing file names and extensions instead of using `Path` objects.
