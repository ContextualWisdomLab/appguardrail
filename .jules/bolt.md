...
**Learning:** In the reviewed Python 3.11.15 microbenchmark, an explicit loop assigning directly to a local dictionary (`seen[item] = None`) was 17.7%–64.5% faster than `dict.fromkeys(item for ...)` across the measured empty, unique, duplicate, and duplicate-with-empty inputs. Both forms still create a dictionary; the explicit loop avoids creating the generator object and its frame. These measurements are workload- and runtime-specific, not a general performance guarantee.
**Action:** For this measured hot path, use the explicit loop while preserving insertion order and filtering semantics. Re-benchmark the production workload and supported Python versions before generalizing the result.
