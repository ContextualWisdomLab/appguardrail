...
## 2024-05-24 - Avoid os.path.splitext in hot loops
**Learning:** In hot loops checking thousands of files, using `os.path.splitext` introduces unnecessary overhead due to internal checks for drive letters and slashes.
**Action:** Use fast-path string manipulation like `str.rfind('.')` for extension extraction to improve performance.
