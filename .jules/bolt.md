...
## 2026-07-30 - Optimizing Path Instantiations in List Comprehensions
**Learning:** Initializing large collections of `Path` objects upfront using list comprehensions (e.g., `[Path(file) for file in files]`) inside performance-critical paths adds severe object allocation and string parsing overhead.
**Action:** Iterate exactly once over file generators. Perform computations (like name and extension parsing) inline and conditionally instantiate `Path` objects only when absolutely necessary, using fast string methods (`os.path.basename`, `os.path.splitext`) as fallbacks.
