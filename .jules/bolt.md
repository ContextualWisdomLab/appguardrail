...
## 2024-05-18 - Avoid generator comprehensions in `dict.fromkeys`
**Learning:** In hot loops, Python generator comprehensions passed directly into `dict.fromkeys(...)` carry substantial overhead due to generator frame creation and iteration overhead, which can be avoided by unrolling them into an explicit `for` loop that mutates a local dictionary (`seen = {}`).
**Action:** Always prefer explicit loop iterations modifying a local lookup table when deduplicating elements sequentially in performance-critical code paths.
