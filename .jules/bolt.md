...
## 2024-11-21 - Optimize string fast-paths before re.finditer and unroll generator expressions
**Learning:** Using native C-level string operations (`"[" not in message`) before invoking `re.finditer` prevents expensive C-level instantiation overhead when the pattern cannot possibly match. Similarly, generator expressions inside `any()` (e.g. `any(token in string for token in (...))`) introduce significant iterator and function call overhead in Python. Unrolling them into a simple chain of `or` conditions (e.g., `"token1" in string or "token2" in string`) provides massive speedups (3x-5x) for hot paths.
**Action:** Use native string pre-checks before running expensive regex operations like `re.finditer`. Avoid `any()` with generator expressions in performance-critical hot loops; unroll them into explicit boolean `or` chains instead to leverage Python's fast-path short-circuiting.
