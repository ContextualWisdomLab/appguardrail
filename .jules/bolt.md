...
## 2024-11-20 - Reduce dict.fromkeys generator overhead
**Learning:** Using `dict.fromkeys()` with generator expressions incurs significant generator overhead and frame allocation in hot paths compared to explicit loops building a dictionary (`seen = {}`).
**Action:** Replace `dict.fromkeys(item for ...)` with explicit loops and dictionary assignments (`seen[item] = None`) when deduplicating items while maintaining order in high-throughput functions.
