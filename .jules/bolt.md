...
## 2024-08-26 - Optimized AppGuardrail Dashboard Finding Filter Performance
**Learning:** In the Javascript frontend dashboard (`scanner/dashboard/index.html`), multiple array iterations mapping and filtering large finding sets can have a huge performance impact when run on every key press in the search bar. Chained \`.map().filter().filter().sort()\` loops over the findings list up to 4 times and unnecessarily casts elements to strings even when they don't match criteria.
**Action:** Replace array iteration chains with a single \`for\` loop to reduce passes from 4 to 1. Cache the lookup indices in an object mapping (\`SEV_IDX\`) instead of repeatedly using \`indexOf\` on a severity string array, and avoid creating large combined strings for search checks if a faster short-circuit check on severity fails. Also hoist formatting strings and only execute toLowerCase conversion once per relevant record instead of across the entire dataset blindly.
