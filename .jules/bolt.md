...
## 2024-05-31 - [Optimize exclusive string prefixes]
**Learning:** Checking mutually exclusive string prefixes with consecutive `if` statements instead of `if/elif` wastes processing time on redundant checks.
**Action:** Always use `elif` for mutually exclusive conditions, especially in hot paths iterating over string prefixes (e.g., using `startswith()`).
