...
## 2024-05-20 - String manipulation for fast extension extraction
**Learning:** In hot loops checking thousands of files (e.g., file discovery), avoid using `os.path.splitext` for extension extraction, as its internal checks for drive letters and slashes introduce unnecessary overhead.
**Action:** Instead, use fast-path string manipulation like `str.rfind('.')` to improve performance.
