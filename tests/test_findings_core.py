...
class _Unstringifiable:
    def __str__(self):
        raise TypeError("string conversion denied")


def test_finding_helpers_fail_closed_for_unhashable_metadata():
    finding = {
        "severity": [],
        "category": {},
        "rule_id": [],
    ...
    assert severity_counts([finding]) == {
        "CRITICAL": 0,
        "HIGH": 0,
        "WARNING": 0,
        "INFO": 1,
    ...
    assert finding_sort_key(finding) == (4, "{}", "[]")
    assert safe_report_snippet({"key": "value"}) == "{'key': 'value'}"


def test_normalize_finding_falls_back_when_string_conversion_is_rejected():
