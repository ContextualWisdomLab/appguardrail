...
def test_has_current_head_changes_requested_from_opencode_agent():
    pr = {
        "headRefOid": "commit123",
        "reviews": {
            "nodes": [
    ...
    assert pr_review_merge_scheduler.has_current_head_changes_requested(pr) is True


def test_has_current_head_changes_requested_from_github_actions_bot():
    """github-actions[bot] is the GITHUB_TOKEN fallback when the OpenCode OIDC
    app-token exchange fails.  The scheduler must recognise its REQUEST_CHANGES
    to prevent infinite re-dispatch loops when all OpenCode agents fail or when
    Strix CI checks fail and the gate posts REQUEST_CHANGES via GITHUB_TOKEN."""
    pr = {
        "headRefOid": "commit123",
        "reviews": {
            "nodes": [
...
def test_has_current_head_changes_requested_github_actions_bot_old_commit():
    """REQUEST_CHANGES from github-actions[bot] on an old commit must not block."""
    pr = {
        "headRefOid": "commit123",
        "reviews": {
            "nodes": [
...
def test_has_current_head_approval_not_from_github_actions_bot():
    """github-actions[bot] APPROVE must NOT count as an OpenCode approval to
    prevent any other workflow from accidentally triggering auto-merge."""
    pr = {
        "headRefOid": "commit123",
        "reviewDecision": "REVIEW_REQUIRED",
        "reviews": {
