...
### 추가
- `appguardrail scan --baseline <path>` — baseline(이전 `--findings-json` 산출물) 대비 **신규 findings에만** deploy 게이트를 실패시킵니다. 기존 부채가 있는 레포도 벽 같은 실패 없이 도입할 수 있고, 새로 유입되는 취약점만 차단합니다. 지문(fingerprint)은 rule+file+내용 기반이라 무관한 편집으로 line이 밀려도 이미 수용한 finding이 다시 뜨지 않습니다.
