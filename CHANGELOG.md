...
### 보안
- Gap baseline에 #1129의 password-indirection FN과 Claude-plugin artifact budget·`source.path` symlink 결함의 RED→GREEN 근거를 추가했습니다. Exact `e33d9a22…`는 psql colon bind를 `.sql`/`.psql`로 제한하고 JS/TS literal finding을 보존하며, 각 read를 남은 aggregate budget+1 byte로 제한한 inventory 하나를 digest/license/source/hook/receipt가 공유합니다. `source.path`의 모든 구성요소는 symlink/stat failure를 fail closed로 처리합니다. Focused 43/43, coverage owner 26/26와 detector 664/664, 전체 1,092/1,092, 문서 11/11이 GREEN이고 review thread 8개는 해결됐습니다. #998 parent integration과 hosted exact-head Checks·독립 review가 없어 #1129는 Draft입니다.
