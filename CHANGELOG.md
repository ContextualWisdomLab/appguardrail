...
### 보안
- Gap baseline에 #1314 console scalar/selector trust-boundary 수선을 결속했습니다. Predecessor `6e8d9b7e…`에서 raw `data-id` attribute escape와 raw `total` markup emission을 executable Node regression으로 재현했고, exact `ec189f3e…`/tree `81f4a552…`는 stats/trend/history count를 non-negative safe integer로 제한하고 ID를 escape하며 severity own-property만 허용합니다. RED 1/0 → GREEN 2/2이고, workflow 9개 queued·승인 0개이므로 mergeable Draft입니다.
