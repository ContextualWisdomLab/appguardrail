...
### 변경
- 대시보드 파일 업로드 프록시 버튼 패턴 적용 — 네이티브 브라우저의 파일 업로드 요소(`<input type="file">`)가 지닌 브라우저 간 스타일 불일치 문제와, 동일한 파일을 연속으로 선택할 때 `change` 이벤트가 무시되는 문제를 수정했습니다. 시각적으로 숨겨진 상태(`sr-only`)에서 동작하는 원본 파일 요소 대신 커스텀 프록시 버튼(`Upload File`)을 배치해 키보드 포커스와 접근성을 보존하고 일관된 인터페이스를 제공합니다.
