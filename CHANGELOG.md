...
### 사용자 경험 (UX)
- 파일 업로드 CTA를 native `<input type="file">`와 연결된 styled `<label>`로 구성해 운영체제 file picker 동작을 유지하고, 키보드 초점 표시와 접근성 트리 노출을 보존했습니다. JavaScript click proxy는 사용하지 않으며, dashboard CSS의 literal `\\n` 구분자도 실제 줄바꿈으로 교정했습니다.
