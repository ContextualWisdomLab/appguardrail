...
- `scripts/ci/pr_review_merge_scheduler.py`의 조건 분기와 GitHub GraphQL API 응답 모킹 테스트를 추가해 `inspected_pr`, `opencode_in_progress` 등 PR 병합 스케줄러 상태 전이를 검증했습니다.

### 변경
- `scanner/cli/vibesec.py` 전체 코드에 대한 테스트 커버리지 100% 달성 및 기존 기능의 안정성 확보 검증.
- 테스트하기 어렵거나 환경에 의존적인 일부 예외 처리 블록, 심볼릭 링크 예외, 그리고 `__name__ == "__main__"` 블록에는 커버리지 의도를 명확히 하기 위해 `pragma: no cover` 주석을 적용했습니다.
