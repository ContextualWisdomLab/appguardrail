...
### 보안
- SSRF/LFI 취약점 추가 보호 — `urllib.request.urlopen`이 기본적으로 리다이렉션을 따라가는 특성을 이용하여 외부 공격자가 검증된 URL에서 내부 네트워크나 로컬 파일(`file://`)로 리다이렉트시키는 우회 공격을 방지했습니다. 이제 커스텀 `NoRedirectHandler`를 사용하여 모든 리다이렉트를 차단합니다.
