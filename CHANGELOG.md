...
- Gap baseline에 #1304 workflow-lifecycle detector의 fresh review 수선을 결속했습니다. Exact `8512ab80…`/tree `05d14b3d…`는 best-effort failure-evidence write의 `OSError`가 이미 진행 중인 원래 실패를 가리지 않아야 한다는 이유를 한 줄로 명시합니다. Parent 대비 1 ahead / 0 behind인 comment-only delta이며 runtime 동작은 그대로입니다. Review thread 4개는 모두 해결됐지만 새 workflow 9개가 queued/pending이고 current-head 독립 승인이 없어 Ready·mergeable 상태만 유지합니다.
