...
### 보안
- Python 명령 주입 탐지가 `os.system`·`os.popen`의 암묵적 셸 실행과 `subprocess(..., shell=True)`를 정확히 구분해 설명하고, 중첩된 명령 생성 호출 뒤의 `shell=True`까지 탐지하도록 보강했습니다. 각 API와 비셸 `subprocess` 음성 사례를 독립 회귀로 검증합니다.
