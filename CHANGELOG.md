...
- Gap baseline에 #1304 workflow-lifecycle detector의 fresh security-review 수선을 결속했습니다. Exact `6cdc231a…`/tree `5b6364ee…`는 regex가 허용한 `git[ \t]+commit`을 literal `git commit` prefilter가 차단하던 scanner-level FN을 교정합니다. Prefilter는 `commit` 후보 토큰만 요구하고 기존 구조 regex가 write 권한·workflow mutation·commit·push 전체를 계속 판정합니다. RED 1 failed / 6 passed에서 focused 7/7, 전체 1,067/1,067, compileall·diff check GREEN으로 전환했고 review thread 5개는 모두 해결됐습니다. 새 exact-head workflow 9개가 queued/pending이고 current-head 독립 승인이 없어 Ready·mergeable 상태만 유지합니다.
