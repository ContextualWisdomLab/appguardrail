...
### 성능 후보
- `severities_at_or_above`의 고정 severity 결과를 사전 계산하되 호출자 변이 격리를 위해 새 `set`을 반환합니다. 정확한 threshold·invalid fallback·mutation isolation 계약은 테스트로 고정했으며, 재현 가능한 median/p95 및 실제 호출 경로 측정 전에는 wall-clock 개선률을 주장하지 않습니다.
- `_merge_references`는 명시적 loop로 first-seen ordering·중복 제거·빈 값 filtering을 유지합니다. 계약은 focused test로 고정했으며 실제 호출 경로 측정 전에는 material performance 개선을 주장하지 않습니다.
