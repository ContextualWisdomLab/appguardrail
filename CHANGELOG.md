...
### 보안
- Gap baseline에 #1260의 반복 동시 severity-guard 삭제와 ordinary-child 복구를 결속했습니다. Concurrent `086fe9ea…`와 `02a5e51a…`가 각각 own-property selector·실행 회귀·CHANGELOG·RCA를 삭제하고 raw `SEV[f.severity]` lookup을 재도입했으나, exact `426d3068…`가 네 authoritative blob을 복구해 이전 GREEN tree `d2809387…`와 byte-identical합니다. 이전 GREEN head보다 4 ahead / 0 behind이고 effective diff는 0이며, 새 workflow 9개가 queued라 과거 승인·Checks를 이전하지 않고 Draft를 유지합니다.
