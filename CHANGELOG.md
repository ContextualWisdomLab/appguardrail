...
### 보안
- Gap baseline에 #1326 reference-extraction performance successor의 stale-prerequisite 수선을 결속했습니다. Exact `a823daaa…`/tree `8f8697b5…`는 original #1326 `52502df8…`와 #976 semantic contract `24d0a000…`의 ordinary two-parent integration이며 explicit-loop delta, bracketed ID contract, no-reference scanner bypass, exclusive classification, 회귀와 CHANGELOG를 함께 보존합니다. RED 1/1에서 focused 9/9, 전체 1,005/1,005 GREEN으로 전환했고 same-host 9×200,000-call medians를 exact parent와 비교했습니다. Hosted workflow 9개가 queued/pending이고 review가 없어 Draft입니다.
- 중복 #1240 exact `2102d3e0…`를 #1217 exact `a7f24c37…`/tree `4cb263bd…`의 verified complete successor carryover로 unmerged 종료했습니다. #1217은 persistence-boundary validation, API 400, explicit null/empty deletion을 모두 보존하면서 canonical `pinned_https` transport로 DNS 단일 결정, non-global peer 거부, TLS hostname, 307/308 redirect, malformed-port와 missing-key fail-closed 계약까지 완성합니다. Current child는 이전 verified owner와 tree-identical하며 exact-tree focused 79/79, 전체 1,029/1,029, compileall·conflict-marker·diff check가 통과했습니다. 새 hosted workflow 9개는 queued/pending이고 latest review가 `CHANGES_REQUESTED`라 #1217은 Draft입니다.
- 중복 #1243 exact `ab96a9a9…`를 #1314 exact `33935474…`/tree `e2388d60…`의 verified complete successor carryover로 unmerged 종료했습니다. #1314는 scan-ID escaping과 dashboard-security 회귀를 보존하고 stats/trend/history 전체의 non-negative safe-integer 경계, own severity-key admission, hostile scalar 실행 회귀, scoped CHANGELOG로 강화합니다. Current child는 `safeCount` 전환에 맞게 predecessor assertion 두 곳을 교정했으며 focused 4/4, 전체 1,003/1,003, compileall·conflict-marker·diff check가 통과했습니다. 소급 `.jules` prototype-mutation 주장은 실행 증거가 아니므로 승계하지 않았고 #1314의 Draft·Checks·review gate는 유지합니다.
- 중복 #1328 exact `9133987f…`를 #1292 exact `5f8cc2cf…`/tree `fa47d924…`의 verified complete successor carryover로 unmerged 종료했습니다. #1292는 fork/non-fork·language·default-branch·private·support 집계를 같은 단일 loop에서 수행하는 production delta를 보존하고, 별도 AST traversal regression을 추가합니다. Focused 5/5, 전체 1,002/1,002, compileall·conflict-marker·diff check가 통과했습니다. 측정 없는 소급 `.jules` blanket performance 문구는 유효 delta가 아니므로 승계하지 않았고 #1292의 Draft·hosted Checks·review gate는 유지합니다.
- #1277 exact `b950f57f…`와 #1286 exact `75e2373f…`를 #1314 exact `454741cf…`/tree `81f4a552…`의 verified complete successor carryover로 unmerged 종료했습니다. #1314는 scan ID escaping, stats/trend/history 전체의 non-negative safe-integer 경계, own severity-key selection을 더 강한 형태로 보존하고 hostile-input 실행 회귀와 scoped CHANGELOG까지 추가합니다. 두 predecessor의 소급 `.jules` prototype-pollution/XSS 문구는 attacker-controlled prototype mutation을 입증하지 못해 유효 delta에서 제외했습니다. Fresh exact-tree 회귀 2/2, compileall·conflict-marker·diff check가 통과했으며 #1314의 Draft·Checks·review gate는 그대로입니다.
- 중복 #1330을 #1269의 verified complete successor carryover로 unmerged 종료했습니다. #1330 exact `92fcc78a…`/tree `5d28bcfe…`의 유효 production delta는 동일한 insertion-time `rule_indices`뿐입니다. #1269 exact `7ceae5e0…`/tree `020b73e2…`는 이를 보존하면서 repeated-rule `[0,0,1]` 회귀, scoped CHANGELOG, 10,000 findings/1,000 rules/7-run 측정, 정확한 O(M×R)→O(M+R) 모델과 #1273 malformed-URI fail-closed prerequisite까지 완전히 승계합니다. #1269의 Draft·Checks·review gate는 그대로입니다.
- Gap baseline에 #1304 workflow-lifecycle detector의 연속 security-review 수선을 결속했습니다. Exact `7eb4579a…`/tree `d86d531a…`는 regex가 허용한 `git[ \t]+commit`과 `git[ \t]+push`를 literal phrase prefilter가 차단하던 두 scanner-level FN을 교정합니다. Prefilter는 `commit`, `git`, `push` 후보 토큰만 요구하고 기존 구조 regex가 write 권한·workflow mutation·commit·push 전체를 계속 판정합니다. 최신 RED 2 failed / 5 passed에서 focused 7/7, 전체 1,067/1,067, compileall·diff check GREEN으로 전환했고 inline review thread 5개는 모두 해결됐습니다. 새 exact-head workflow 9개가 queued/pending이고 current-head 독립 승인이 없어 Ready·mergeable·ordinary auto-merge 상태만 유지합니다.
- Gap baseline에 #1314 console scalar/selector trust-boundary 수선과 동시 회귀 복구를 결속했습니다. Generated child `d9346e7a…`가 safe-integer·own-property 경계, 실행 회귀와 CHANGELOG를 삭제해 executable **2/2 RED**를 재현했으며, ordinary child `454741cf…`/tree `81f4a552…`는 검증된 네 blob을 정확히 복원합니다. GREEN 2/2이고 workflow 9개 queued·current-head 승인 0개이므로 mergeable Draft입니다.
- #1269 SARIF rule-index successor를 current #1273 위로 ordinary two-parent restack했습니다. Exact `7ceae5e0…`/tree `020b73e2…`는 parent 대비 5 ahead / 0 behind이며 #1273 malformed-URI fail-closed 계약과 #1269 insertion-time index delta를 함께 보존합니다. Focused 7/7과 compile 검사는 통과했지만 최초 조회에서 hosted workflow와 qualifying review가 없어 Draft입니다.
- Gap baseline에 #1273 SARIF `helpUri` fail-closed 수선을 결속했습니다. Predecessor `e97f7b20…`는 malformed bracket host에서 SARIF 생성을 중단하고 whitespace/control/incomplete-percent reference를 URI metadata로 허용했습니다. RED 2 failed / 5 passed 뒤 current exact `4c45491d…`가 invalid candidate를 건너뛰고 다음 valid HTTP(S) reference를 평가하며 원문을 `help.text`에 보존합니다. Focused 7/7과 compile은 통과했지만 workflow 9개와 fresh review가 대기 중이므로 Draft입니다.
- Gap baseline에 #1260의 active-writer severity-guard 회귀를 결속했습니다. Concurrent `086fe9ea…`, `02a5e51a…`, `0f78f678…`가 동일하게 own-property selector·실행 회귀·CHANGELOG·RCA를 삭제하고 raw lookup을 재도입했습니다. 두 ordinary restoration은 검증 GREEN tree를 복구했지만 다시 덮였으므로 현재 exact `0f78f678…`/tree `4f55cd8f…`는 known-regressed Draft로 기록하고, writer control 해결 전 무한 push loop를 중단합니다.
- Gap baseline에 #1324의 동시 Python detector 회귀 복구를 결속했습니다. Concurrent `bdef14c0…`는 dynamic `shell=enabled`를 놓치고 UTF-8 앞 call의 exact offset 대신 line start를 보고하며 56줄 regression과 CHANGELOG를 삭제했습니다. RED positive 5/6·UTF-8 offset false 뒤 `d8e5af78…`가 verified tree를 복원했고, current exact `658814ba…`까지 두 tree-identical child가 뒤따랐습니다. Focused positive 6/6, negative 0, offset/fallback/compile GREEN이며 workflow 9개 queued, 승인 0개라 Draft입니다.
- Gap baseline에 current #1217 owner와 #1327 detector boundary 복구를 결속했습니다. Concurrent `f1ebd72e…`가 canonical production/CLI를 수정하고 154-line persistence fixture와 detector CHANGELOG를 삭제했으나, ordinary two-parent `de921817…`가 current owner와 concurrent child를 모두 승계하면서 verified detector-only tree를 복원했습니다. Current exact `2437561e…`까지 두 tree-identical child가 뒤따랐으며 owner 대비 9 ahead / 0 behind, effective files 3개, workflow 9개 queued, 승인 0개라 Draft입니다.
