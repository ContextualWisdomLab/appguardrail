...
### 사용자 경험 (UX)
- 대시보드 테이블 헤더 접근성 및 동적 텍스트 복수형 개선 — 스크린 리더 사용자가 테이블 데이터를 명확히 이해할 수 있도록 `console.html`과 `index.html`의 주요 테이블 `<th>` 요소에 `scope="col"`을 추가하여 열(column)과의 연관성을 명시했습니다. 또한, 터미널 출력 및 UI에 표시되는 텍스트(예: "scan(s)", "findings")를 하드코딩된 형태에서 벗어나 항목 수에 따라 삼항 연산자를 이용해 단수/복수로 동적 출력하도록 변경하여 화면의 품질을 향상시켰습니다.
