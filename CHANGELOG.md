...
### 보안
- Webhook URL 검증이 host 없는 HTTP(S) 입력을 DNS compatibility 경로에서 허용하던 fail-open을 수정했습니다. Control-plane API·직접 persistence·CLI validator가 `http://`, `https://`, userinfo-only URL을 일관되게 거부하며, 빈 문자열의 webhook 해제 계약은 유지합니다.
