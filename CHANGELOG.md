...
### 보안
- 중복 #1240 exact `2102d3e0…`를 #1217 exact `49bf978e…`/tree `4cb263bd…`의 verified complete successor carryover로 unmerged 종료했습니다. #1217은 persistence-boundary validation, API 400, explicit null/empty deletion을 모두 보존하면서 canonical `pinned_https` transport로 DNS 단일 결정, non-global peer 거부, TLS hostname, 307/308 redirect, malformed-port와 missing-key fail-closed 계약까지 완성합니다. Current exact-tree focused 79/79, 전체 1,029/1,029, compileall·conflict-marker·diff check가 통과했습니다. Hosted workflow 8개는 GREEN이고 CodeQL은 queued이며 latest review가 `CHANGES_REQUESTED`라 #1217은 Draft입니다.
