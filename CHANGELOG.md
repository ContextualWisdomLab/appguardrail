...
### 보안
- Gap baseline에 #1129의 password-indirection FN과 Claude-plugin artifact budget 결함의 RED→GREEN 근거를 추가했습니다. Exact `546ffaaf…`는 psql colon bind를 `.sql`/`.psql`로 제한하고 JS/TS literal finding을 보존하며, entry/file/byte-bounded inventory 하나를 digest/license/source/hook/receipt가 공유합니다. NOTICE evidence, directory-only fanout, single oversized payload 회귀를 포함해 focused 40/40·전체 1,089/1,089·문서 11/11이 GREEN이고 review thread 8개는 해결됐습니다. #998 parent integration과 hosted exact-head Checks·독립 review가 없어 #1129는 Draft입니다.
