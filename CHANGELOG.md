...
### 보안
- SSRF 완화 — `urllib.request.urlopen` 대신 `HTTPRedirectHandler` 리다이렉션을 비활성화한 커스텀 opener를 사용하여 CLI 원격 보고(push) 및 Control Plane 웹훅 전송 시 악의적인 HTTP 리다이렉트를 통한 SSRF(Server-Side Request Forgery) 및 내부망 우회 공격을 방지하도록 수정했습니다.
