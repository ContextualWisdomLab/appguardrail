...
### 보안
- Gap baseline에 #1324의 Python shell-call raw-regex FP/FN 수선을 결속했습니다. Exact `b7d9c6a7…`는 comments/string literals와 `shell=False/0/None`을 음성으로 유지하고, 실제 `os.system` 및 non-false `subprocess(..., shell=...)` AST call을 exact source offset으로 보고합니다. Predecessor RED 2/2, compile 2/2, focused semantic contracts 4/4이며 hosted workflow 9개와 독립 review는 대기 중이므로 Draft입니다. Aliases/wrappers/positional shell과 syntax-invalid fallback FP는 명시적 잔여 경계입니다.
