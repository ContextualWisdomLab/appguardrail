...
### 사용자 경험 (UX)
- 대시보드 finding reference 링크는 새 탭 전환을 화면과 보조 기술에 함께 알리고, 제품 소유 `.external-reference` CSS class·`noopener noreferrer`·decorative SVG 경계를 사용합니다. Assertion 없는 독립 Playwright smoke artifact는 executable acceptance로 오인되지 않도록 제거했습니다.
