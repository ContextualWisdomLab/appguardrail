...
def fast_detect_language_axes(files):
    ...
    seen_suffixes = set()
    seen_names = set()
    get_lang = LANGUAGE_BY_EXTENSION.get
    for file_path in files:
        if not isinstance(file_path, str):
            name = file_path.name
            suffix = file_path.suffix.lower()
        ...
        # Stop early if we have found all possible unique languages we care about
        # Currently AppGuardrail has about 12 supported axes, capping at 12 is a huge win for massive repos
        if len(languages) >= 12:
            break
    return languages
